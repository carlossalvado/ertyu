...
import { MessageCircle, CheckCircle, Bot, QrCode } from 'lucide-react';
...
Se não souber algo específico, sugira que o cliente entre em contato diretamente.`);
  ...
  const loadUserData = async () => {
    ...
    try {
      ...
      // Carregar configurações do agente IA
    ...
    try {
      ...
      if (res.ok) {
        if (data.qrCode) {
          ...
          setQrCode(null);
          setMessage('WhatsApp conectado com sucesso!');
          loadUserData(); // Recarrega configuração
        } else {
  ...
  const handleConnect = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage('Informações salvas! Agora configure o WhatsApp.');
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center space-x-3 mb-6">
          ...
          <h1 className="text-3xl font-bold text-gray-800">Configuração WhatsApp AI Agent</h1>
        </div>
